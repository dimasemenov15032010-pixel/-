...
SERVERS = {
    ...
    86: "Сургут", 87: "Подольск", 88: "Магадан", 89: "Череповец", 90: "Норильск",91: "Астана",
}
...
def get_help_text() -> str:
    return (
        ...
        "КОМАНДЫ ЕПТ\n"
        "/addacc (ДОБАВИТЬ АККАУНТ) СЕРВЕР/НОМЕР НИК ПАРОЛЬ(/addacc 91 Mirak_Lox miraklox123)\n"
        "
/list /panel (КОМАНДЫ ВЫЗОВА СПИСКА СЕРВЕРОВ\n"
        "/accedit (ИЗМЕНЕНИЕ НИКА И ПАРОЛЯ) ПРИМЕР:(/accedit 10(номер акка) Mirak_Lox miraklox123\n"
        "/accdel номер_аккаунта — удалить аккаунт (с подтверждением)\n"
        "/search(ПОИСК АККАУНТОВ НА ВВЕДЕННОМ СЕРВЕРЕ) Пример:( /search 90 ИЛИ норильск)\n\n"
        "<b>Учёт домов</b>\n"
        "/house(ДОБАВИТЬ ДОМ) ПРИМЕР:(/house номер аккаунта дни оплаты владелец ) ИЛИ /house  сервер ник пароль и дни оплаты\n"
        "/hedit(ИЗМЕНИТЬ ПРОДЛИТЬ ОПЛАТУ) ПРИМЕР (/hedit 1(номер дома ) дни оплаты)\n"
        "/hdel (УДАЛЕНИЕ ДОМА ИЗ СПИСКА) ПРИМЕР :(/hdel номер дома\n"
